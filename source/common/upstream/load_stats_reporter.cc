...
LoadStatsReporter::LoadStatsReporter(const LocalInfo::LocalInfo& local_info,
    ...
    : cm_(cluster_manager), stats_{ALL_LOAD_REPORTER_STATS(
      ...
      load_reporting_service_store_(load_reporting_service_store), send_request_latencies_{false} {
...
LoadStatsReporter::~LoadStatsReporter() {
  load_reporting_service_store_.shutdownThreading();
}
...
void LoadStatsReporter::sendLoadStatsRequest() {
  ...
  if (send_request_latencies_) {
    ...
    load_reporting_service_store_.mergeHistograms([this, metrics_to_cluster_map] {
      Stats::MetricSnapshotImpl snapshot(load_reporting_service_store_);
