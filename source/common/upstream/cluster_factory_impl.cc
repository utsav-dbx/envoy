...
std::pair<ClusterSharedPtr, ThreadAwareLoadBalancerPtr> ClusterFactoryImplBase::create(
  ...
  ClusterFactoryContextImpl context(
      cluster_manager, stats, load_report_stats_store, tls, std::move(dns_resolver), ssl_context_manager, runtime, random,
...
ClusterFactoryImplBase::create(const envoy::config::cluster::v3::Cluster& cluster,
  ...
  auto&& load_report_stats_store = context.loadReportStatsStore();
  if (load_report_stats_store.has_value()) {
    new_cluster_pair.first->setLoadReportStatsScope(generateStatsScope(cluster, *load_report_stats_store));
  }
