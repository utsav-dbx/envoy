...
ClusterInfoImpl::ClusterInfoImpl(
    ...
    : runtime_(runtime), name_(config.name()), type_(config.type()),
      ...
      // FIXME(utsav)
      load_report_stats_(true ? absl::make_optional<ClusterLoadReportStats>(
                                                              generateLoadReportStats(load_reporting_service_store)
      ) : absl::nullopt),
...
ClusterImplBase::ClusterImplBase(
  ...
  info_ = std::make_unique<ClusterInfoImpl>(
      ...
      std::move(stats_scope), load_reporting_service_store, added_via_api, factory_context.messageValidationVisitor(),
      factory_context);
